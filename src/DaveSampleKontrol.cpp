...
void handleRoot() {
    ...
    const char* html =
        ...
        "   document.getElementById('status').innerText = 'IP: '+j.ip+' | Mode: '+j.wifi_mode+' | RSSI: '+j.rssi+' | Uptime ms: '+j.uptime_ms+' | LED: '+j.led+' | S1: '+j.s1+' | S2: '+j.s2+' | S3: '+j.s3+' | S4: '+j.s4;"
        "  }catch(e){document.getElementById('status').innerText='Error fetching status';}}"
        "document.getElementById('toggle').addEventListener('click', async ()=>{"
        "  try{const r=await fetch('/api/toggle', {method:'POST'}); await fetchStatus();}catch(e){alert('Error');}});"
